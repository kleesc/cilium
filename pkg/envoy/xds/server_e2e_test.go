...
func (s *ServerSuite) TestRequestAllResources(c *C) {
	...
	server := NewServer(map[string]ObservableResourceSet{
		typeURL: cache,
	}, TestTimeout)
	...
	req.VersionInfo = resp.VersionInfo // ACK the received version.
	...
	cache.Upsert(typeURL, resources[0].Name, resources[0])
	...
	cache.Upsert(typeURL, resources[1].Name, resources[1])
	...
	time.Sleep(CacheUpdateDelay)
	cache.Delete(typeURL, resources[0].Name)
...
func (s *ServerSuite) TestRequestSomeResources(c *C) {
	...
	time.Sleep(CacheUpdateDelay)
	cache.Upsert(typeURL, resources[2].Name, resources[2])
	...
	cache.Delete(typeURL, resources[1].Name)
...
func (s *ServerSuite) TestUpdateRequestResources(c *C) {
	...
	time.Sleep(CacheUpdateDelay)
	cache.tx(typeURL, map[string]proto.Message{
	...
	req.ResourceNames = []string{resources[1].Name, resources[2].Name}
	err = stream.SendRequest(req)
	c.Assert(err, IsNil)
