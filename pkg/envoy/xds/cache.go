...
// resources are available under the same names to all clients..
type Cache struct {
	*BaseObservableResourceSet
...
func NewCache() *Cache {
	return &Cache{
		BaseObservableResourceSet: NewBaseObservableResourceSet(),
...
// changed.
func (c *Cache) tx(typeURL string, upsertedResources map[string]proto.Message, deletedNames []string) {
	c.locker.Lock()
...
// Upsert inserts or updates a resource from this cache by name and increases
// the cache's version number atomically if the resource is actually inserted
// or updated.
func (c *Cache) Upsert(typeURL string, resourceName string, resource proto.Message) {
	c.tx(typeURL, map[string]proto.Message{resourceName: resource}, nil)
}

// Delete deletes a resource from this cache by name and increases the cache's
// version number atomically if the resource is actually deleted.
func (c *Cache) Delete(typeURL string, resourceName string) {
	c.tx(typeURL, nil, []string{resourceName})
}
