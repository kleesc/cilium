...
// sets to access resources. sets maps each supported type URL to its
// corresponding resource set.
func NewServer(sets map[string]ObservableResourceSet, resourceAccessTimeout time.Duration) *Server {
	watchers := make(map[string]*ResourceWatcher, len(sets))
	for typeURL, set := range sets {
		w := NewResourceWatcher(typeURL, set, resourceAccessTimeout)
		set.AddResourceVersionObserver(w)
		watchers[typeURL] = w
	}
...
func (s *Server) processRequestStream(ctx context.Context, streamLog *logrus.Entry, stream Stream,
	...
	for typeURL := range s.watchers {
		...
		selectCases[i].Chan = reflect.ValueOf(quietCh)
	...
	for {
		...
		case reqChIndex: // Request received from the stream.
			...
			requestLog.Info("processing xDS request")
			...
			// the first request or if it's the nonce of the last response.
			if state.nonce == "" || state.nonce == req.GetResponseNonce() {
				if state.pendingWatchCancel != nil {
					// A pending watch exists for this type URL. Cancel it to
					// start a new watch.
					requestLog.Debug("cancelling pending watch")
					state.pendingWatchCancel()
				}

				// Ensure that the version info is an uint64 value, since we
				// only return integer versions in Cilium.
				var versionInfo *uint64
				if req.GetVersionInfo() != "" {
					versionInfoVal, err := strconv.ParseUint(req.GetVersionInfo(), 10, 64)
					if err != nil {
						requestLog.Error("non-uint64 version info in xDS request")
