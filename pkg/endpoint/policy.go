...
func (e *Endpoint) regenerateConsumable(owner Owner, labelsMap *LabelsMap,
	...
	if e.L4Policy != c.L4Policy || e.LabelsMap != labelsMap {
		...
		// We need to know which ones are L4-only by checking
		// if there's any L4-only rules that doesn't match a L3-L4 rule.
		...
		// If there were no modifications to the L3L4 policy lets copy the
